...
public class MemberController {
    ...
    public String showLoginPage(Model model) {
        // 로그인할 때 사용할 Dto 전달
        model.addAttribute("memberLoginDto", new AccountLoginDto());
    ...
    public String showSignUpPage(Model model) {
        // 회원가입 시 사용할 Dto 전달
        model.addAttribute("memberSignUpDto", new AccountSignUpDto());
        return "member/member-signup";
    }

    @PostMapping("/signup")
    public String createNewMember(@Valid AccountSignUpDto accountSignUpDto, BindingResult bindingResult, Model model) {
        // 주석 추가2
        if (bindingResult.hasErrors() || memberService.existMemberCheck(accountSignUpDto)) {
        ...
        Account account = memberService.saveNewMember(accountSignUpDto);
